...
(async function() {
  ...
  try {
    ...
    if (!res.ok) throw new Error('team.json failed to load');
    const team = await res.json();
    render(team);
  } catch (err) {
    // Fallback: try inline JSON if present (useful for file:// or strict CSP environments)
    try {
      const inline = document.getElementById('teamData');
      if (!inline) throw new Error('no inline team data');
      const team = JSON.parse(inline.textContent || '[]');
      if (!Array.isArray(team) || team.length === 0) throw new Error('inline empty');
      render(team);
    } catch (_) {
      grid.innerHTML = '<p style="color: var(--muted);">Team information failed to load.</p>';
    }
  }
  ...
  function link(icon, href) {
    const a = document.createElement('a'); a.href = href; a.target = '_blank'; a.rel = 'noopener'; a.setAttribute('aria-label', 'link');
